...
size_t Program::RpnNext(Branch& myobj) {
    ...
    // increment then test
    // careful: round toward minus infinity to avoid missing last boundary (because growing step)
	myobj.first_index = myobj.first_index + 1.0;
...
size_t Program::RpnStep(Branch& myobj) {
    ...
    long double step = stack_.value<Number>(0);
    stack_.pop();
    ...
    if (step <= 0) {
		...
		myobj.first_index = myobj.first_index + step;

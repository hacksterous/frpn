...
bool Lexer::stringToDouble(const char* str, long double& dbl, int base) {
	char* endPtr;
	char temp[30];
	//only base-2/10/16 are allowed
	//std::cout << "anirb: Lexer::stringToDouble -- initial str = " << str << std::endl;
	...
	//std::cout << "anirb: Lexer::stringToDouble -- got final str = " << temp << std::endl;
	dbl = strtold(temp, &endPtr);
	if (*endPtr != '\0') return false; // bad number	
	return true; // good number
}

bool Lexer::GetNumberAt(const string& entry, size_t idx, size_t& next_idx, int& base, long double& r, char delim) {
	stringstream ss;
	string token;
	int convstat;
	next_idx = idx;
	...
	if (getline(ss, token, delim)) {
		...
		//std::cout << "anirb: Lexer::GetNumberAt -- r = " << r << std::endl;
...
bool Lexer::ParseNumber(const string& entry, size_t idx, size_t& next_idx, vector<SynError>& errors,
	...
	if (GetNumberAt(entry, idx, next_idx, base, r)) {
		//std::cout << "anirb: Lexer::ParseNumber -- got long double = " << r << std::endl;

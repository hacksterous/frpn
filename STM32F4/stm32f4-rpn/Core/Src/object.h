...
struct Number : Object {
    ...
    explicit Number(double value__, int base__ = 10) : Object(kNumber), base(base__), value(value__) {}
    explicit Number(size_t value__, int base__ = 10) : Object(kNumber), base(base__), value(value__) {}
    explicit Number(long value__, int base__ = 10) : Object(kNumber), base(base__), value(value__) {}
    explicit Number(unsigned long value__, int base__ = 10) : Object(kNumber), base(base__), value(value__) {}
    ...
    // precision
    static constexpr int kDefaultPrecBits = 32;
    static constexpr int kDefaultDecimalDigits = 16;
    static int digits;

    // clang-format on
    static ostream& ShowValue(ostream& out, const double& value, mode_enum mode, int digits, int base);
};
...
struct Complex : Object {
    ...
    explicit Complex(complex<double>& value__, int re_base__ = 10, int im_base__ = 10)
        : Object(kComplex), re_base(re_base__), im_base(im_base__) {
        value = value__;
    }
    explicit Complex(double& re__, double& im__, int re_base__ = 10, int im_base__ = 10)
        : Object(kComplex), re_base(re_base__), im_base(im_base__) {
        value.real(re__);
        value.imag(im__);
    }

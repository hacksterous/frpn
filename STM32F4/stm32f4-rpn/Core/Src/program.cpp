...
vector<Program::keyword_t> Program::keywords_ = {
	...
	{kKeyword, "test",	{.prog = &Program::RpnQuit}, ""}, //FIXME: replaced RpnTest
...
RetValue Program::Parse(const string& entry) {
	...
	if (Analyse(entry, keywords_map, elements, errors)) {
		...
		for (Lexer::SynElement& element : elements) {
			switch (element.type) {
				case kNumber:
					//cout << "anirb: Program::Parse -- number = " << int(element.re) << "\r\n";
				...
				case kComplex:
					...
					break;
				case kString:
					//cout << "anirb: Program::Parse -- string = " << element.value << "\r\n";
					push_back(new String(element.value));
					break;
				case kSymbol:
					//cout << "anirb: Program::Parse -- symbol = " << element.value << "\r\n";
					push_back(new Symbol(element.value, element.auto_eval));
					break;
				case kProgram: 
				{
					Program* p = new Program(stack_, heap_, this);
					//cout << "anirb: Program::Parse -- program = " << element.value << "\r\n";
					...
					break;
				case kKeyword:
					//cout << "anirb: Program::Parse -- keyword = " << element.value << "\r\n";
...
RetValue Program::ShowError() {
	...
	// clang-format on
	cout << err_context_ << ": error " << err_ << ": " << errorStrings[err_] << "\r\n";
	switch (err_) {
		case kInternalError:
		case kDeadlyError:
			ret = kDeadlyError;
...
void Program::ShowStack(bool show_separator) {
	//cout << "anirb: Program::Parse -- entered ShowStack...\r\n";
	if (stack_.size() == 1) {
